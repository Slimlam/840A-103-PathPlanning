...
# Print the final path in the specified LineSegment format
if len(full_path) > 1:
    line_segments = []
    for i in range(len(full_path) - 1):
        x1, y1 = full_path[i]
        x2, y2 = full_path[i + 1]
        line_segments.append(f"LineSegment({x1}, {y1}, {x2}, {y2})")
    print("\n" + ", ".join(line_segments))
else:
    print("\nNo valid path to format as LineSegment.")
